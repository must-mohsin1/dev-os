...
- `references/item6-build-learnings.md` — three concrete recovery patterns from the item6 build: worker test counts lie (always re-run the full suite before force-completing), terminal "hang" vs real pytest hang (tee/tail pipelines buffer carriage-return progress lines), and Next.js route conflict recovery (mv to .bak to keep the build green while preserving evidence).
